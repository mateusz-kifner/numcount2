...
class CanvasObject {
  constructor ( x=0, y=0, width=100, height=100, margin=10, padding=10 ) {
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
...
class SquareBoard extends CanvasObject {
  constructor(x=0,y=0,size=600,margin=20){
    ...
    this.text_size = 40;
    this.padding = (this.size - (this.tiles_count*this.tile_size+(2*this.tiles_count*this.margin)))/2;
    this.tiles = [];
  ...
  init(){
    this.tile_template.size = ((this.size/Math.sqrt(this.tiles_count)) -(2*this.tile_template.margin));
    this.current_number = 0; 
    ...
    for(let a=0;a<tiles_sqrt;a++){
      for(let b=0;b<tiles_sqrt;b++){
        ...
        colorMode(HSB);
        this.tiles[this.current_number].color = color(floor(random()*10)*36,70,100,0.8);
        colorMode(RGB);
        this.current_number++;
      }
    }
  ...
  reset(){
    ...
    */
    ...
    for(let a=0;a<this.tiles_count;a++){
      ...
      let x = this.x + this.tiles[a].column*(this.tiles[a].size + 2*this.tiles[a].margin) + this.tiles[a].margin;
      let y = this.y + this.tiles[a].row*(this.tiles[a].size + 2*this.tiles[a].margin) + this.tiles[a].margin;
      this.tiles[a].x = x;
      this.tiles[a].y = y;
  ...
  draw(){
    ...
    for(let a=0;a<this.animations.length;a++){
      if( this.animations[a].frame > 600){
        ...
        if(this.animations[a].stroke){
          this.animations[a].color.setAlpha((sin(this.animations[a].frame/200*TWO_PI - HALF_PI)+1)/2);
          noStroke();
          fill(this.animations[a].color);
          ellipse(this.animations[a].x + this.animations[a].size/2,this.animations[a].y + this.animations[a].size/2,this.animations[a].size+50);
        }
    ...
    for(let a=0;a<this.tiles_count;a++){
      ...
      rect(this.tiles[a].x,this.tiles[a].y,this.tiles[a].size,this.tiles[a].size,this.round_corners);
      fill(0);
      stroke(0,0,0);
      strokeWeight(3);
      if(this.tiles[a].number <= this.win_count)
        text(this.tiles[a].number,this.tiles[a].x+text_left_padding,this.tiles[a].y+text_top_padding,this.tiles[a].size,this.tiles[a].size-text_top_padding);
    }
    ...
    for(let a=0;a<this.animations.length;a++){
      if( this.animations[a].frame > 1050){
        ...
        if(!this.animations[a].stroke){
          this.animations[a].color.setAlpha(map(this.animations[a].frame,0,70,0.8,0.0,true));
          stroke(color(0,map(this.animations[a].frame,0,70,255,0,true)));
          fill(this.animations[a].color);
          rect(this.animations[a].x+this.animations[a].frame,this.animations[a].y + this.animations[a].frame,max(this.animations[a].size-(2*this.animations[a].frame),0.001),max(this.animations[a].size-(2*this.animations[a].frame),0.001),this.round_corners + max(this.animations[a].frame,20));
        }
  ...
  hover(){
    let coll = this.check();
    for(let a=0;a<this.tiles_count;a++){
      if(a === coll) this.tiles[a].hover = true;else this.tiles[a].hover = false;
    }
...
class Menu extends CanvasObject {
  constructor(x = 0,y = 0,width = 600,height = 400,margin = 0,padding = 0){
    ...
    this.button_template = {
      ...
      draw(){
        if(this.shadow>0){
          fill(0,this.shadow);
          noStroke();
          rect(this.x + 6,this.y + 6,this.width,this.height,this.round_corners);
        }
        stroke(0);
        strokeWeight(3);
  ...
  checkButtonHover(){
    let update = false;
    for(let obj of this.buttons){
      if(mouseX < obj.x + obj.width && mouseX > obj.x && mouseY < obj.y + obj.height && mouseY > obj.y){
        if(!obj.hover){
          obj.hover = true;
          update = true;
        }
...
class Timer extends CanvasObject {
  ...
  draw(){
    ...
    for(let i=0;i<10;i++){
      fill(0,2*i);
      ellipse(this.x + this.size/2, this.y + this.size/2, this.size -(2*i));
    }
    fill(255);
    ellipse(this.x + this.size/2, this.y + this.size/2, this.size - this.margin);
    noFill();
    stroke(150);
    ellipse(this.x + this.size/2,this.y + this.size/2,this.size - 2*this.margin);
    fill(70,120,255);
    noStroke();
    let offset_x = (this.size/2 - this.margin)*sin((this.time_ms-1000/2)/1000*TWO_PI);
    let offset_y = (this.size/2 - this.margin)*cos((this.time_ms-1000/2)/1000*TWO_PI);

    ellipse(this.x + this.size/2 + offset_x,this.y + this.size/2 + offset_y,20*this.size/200);
    for(let i=0;i<50;i++){
      stroke(color(map(i,0,50,70,150),map(i,0,50,120,150),map(i,0,50,255,150),map(i,0,50,255,0)));
      strokeWeight(this.size/200*4);
      offset_x = (this.size/2 - this.margin +this.size/200*3)*sin((this.time_ms-1000/2 +i*(1000/300)+(1/this.size)*200)/1000*TWO_PI);
      offset_y = (this.size/2 - this.margin +this.size/200*3)*cos((this.time_ms-1000/2 +i*(1000/300)+(1/this.size)*200)/1000*TWO_PI);
      let offset_x2 = (this.size/2 - this.margin-this.size/200*3)*sin((this.time_ms-1000/2 +i*(1000/300)+(1/this.size)*200)/1000*TWO_PI);
      let offset_y2 = (this.size/2 - this.margin-this.size/200*3)*cos((this.time_ms-1000/2 +i*(1000/300)+(1/this.size)*200)/1000*TWO_PI);
      line(this.x + this.size/2+offset_x,this.y + this.size/2 + offset_y,this.x + this.size/2 + offset_x2,this.y + this.size/2 + offset_y2);
    }
    fill(0);
    stroke(0);
    strokeWeight(3);
    ...
    let text_time = (this.time_s<3 && this.time_ms>0) ? (this.time_s + (this.time_ms/1000)).toFixed(2):this.time_s;
...
class Game {
  ...
  draw () {
    if(this.update){
      ...
      case this.states.WAITING:{
        ...
        this.heartui.draw();
        noStroke();
        if(this.waiting_timer.time_s == 0 && this.waiting_timer.time_ms>0){
      ...
      case this.states.GAMELOOP:
        ...
        this.heartui.draw();
        if(this.square_board.current_number-this.square_board.tiles_count == this.square_board.win_count){
        ...
        if(this.hint_time<millis()){
          for(let tile of this.square_board.tiles){
            if(tile.number == (this.square_board.current_number-this.square_board.tiles_count+1)){
              this.square_board.animations.push(Object.assign({},tile));
              this.square_board.animations[this.square_board.animations.length-1].frame =0;
              this.square_board.animations[this.square_board.animations.length-1].stroke = true;
            }
          }
          this.hint_time = millis() + 10000;
        }
  ...
  check(){
    switch(this.state){
    case this.states.MENU:
      if(this.main_menu.checkButtonHover()){this.update+=1;}
      break;
    case this.states.WIN:
      if(this.win_menu.checkButtonHover()){this.update+=1;}
      break;
    case this.states.LOSE:
      if(this.lose_menu.checkButtonHover()){this.update+=1;}
      break;
    }
  }
  ...
  touchStarted() {
    ...
    case this.states.MENU:
      for (let obj of this.main_menu.buttons){
        if(obj.hover) obj.pressed = true; else obj.pressed = false;
      }
      break;
    case this.states.GAMELOOP:{
